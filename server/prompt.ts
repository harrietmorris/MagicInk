...
function promptForMore(title: string, storyString: string,
  prompt: string,
  readingTime?: number,
  readingLevel?: string,
  themes?: string) {
  ...
  const add = `${persona}
   Write a childrens story with the following parameters:
    ...
    - Your response should include the story you are continuing.
    ${guidelines}`
...
export { createPrompt, createContinuationPrompt, promptForMore };
