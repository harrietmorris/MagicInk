...
export async function removeStoryFromProfile(ctx: Context) {
  const { profileId, storyId } = ctx.params;
  ...
  try {
    ...
    console.error('Error removing story from profile', error);
    ctx.status = 400;
    ctx.body = { error: 'Error removing story from profile' };
  }
}
